...
Исключение для публичной границы — Supabase Auth: браузер вызывает его через официальный SDK с публичным anon/publishable key для регистрации, email/password и Google OAuth входа, выхода и запроса recovery-письма. Пароль и Google Client Secret не проходят через API UstaBIM Tools. Одноразовые signup/recovery/OAuth codes обмениваются серверным callback на cookie-сессии; recovery допускает только `/reset-password`, signup — `/app`, OAuth — только внутренние `/app` и `/app/*`. Public key не даёт административных прав и не заменяет обязательные RLS-политики; service role key никогда не попадает в браузер.
...
Первый реализованный workflow чата принимает только текст сообщения через отдельный HTTPS webhook, проверяет HMAC-подпись, timestamp и одноразовый nonce, затем вызывает языковую модель и возвращает один текстовый ответ. Он не управляет пользователями, проектами или правами доступа. Память диалога, RAG и инструменты остаются будущими возможностями.
...
Браузерный viewer на That Open Components и web-ifc открывает локальный IFC, преобразует его в Fragments и отображает в WebGL без отправки исходного файла на сервер. Реализованы навигация камеры, сетка, вписывание модели, raycast-выбор с подсветкой, пространственное дерево, просмотр атрибутов и property sets, линейные измерения и плоскости разреза. В следующих этапах viewer будет загружать IFC по краткоживущему signed URL, скрывать/изолировать элементы и сохранять виды. Viewer не обходит RLS и не хранит сервисные ключи.
...
- Для регистрации и email-входа клиент обращается к публичному Supabase Auth API; signup требует подтверждения адреса, Next.js proxy обновляет cookie, а защищённые server layouts проверяют claims повторно.
- Доступ к проекту и файлу проверяется до выдачи signed URL и до запуска дорогой операции.
- Web application и n8n взаимодействуют по подписанным запросам с ограниченным временем действия и защитой от повторного воспроизведения: `POST /api/chat/message` принимает только `{ content }`, а сервер отправляет n8n `{ message }` с HMAC-SHA256 заголовками timestamp и nonce.
