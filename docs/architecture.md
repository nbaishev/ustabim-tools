...
Браузерный viewer на That Open Components и web-ifc открывает локальный IFC либо IFC проекта, преобразует его в Fragments и отображает в WebGL. Проектный файл загружается владельцем или редактором напрямую в приватный Storage по signed upload URL и выдаётся участнику по короткоживущей signed URL после проверки RLS. Серверная обработка, скрытие/изоляция и сохранённые виды остаются следующими этапами. Viewer не обходит RLS и не хранит сервисные ключи.
