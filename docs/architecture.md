...
Браузерный viewer на That Open Components и web-ifc в текущем первом срезе открывает локальный IFC, преобразует его в Fragments и отображает в WebGL без отправки исходного файла на сервер. Реализованы базовая навигация камеры, сетка и вписывание модели. В следующих этапах viewer будет загружать IFC по краткоживущему signed URL, выбирать элементы, показывать свойства и дерево, скрывать/изолировать, измерять и создавать секущие плоскости. Viewer не обходит RLS и не хранит сервисные ключи.
