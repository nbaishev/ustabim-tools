...
export function GeologyWorkspace() {
  ...
  return <div className="mx-auto max-w-[1500px]">
    ...
    <section className="mt-8 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-200 px-5 py-4"><h2 className="font-semibold text-slate-950">Результат анализа</h2><p className="mt-1 text-xs text-slate-500">Извлечённые сведения требуют проверки профильным специалистом.</p></div>{report ? <div className="grid gap-5 p-5 sm:grid-cols-2"><div className="sm:col-span-2"><h3 className="font-semibold text-slate-900">Сводка</h3><p className="mt-2 text-sm leading-6 text-slate-600">{report.summary || "Сводка не извлечена."}</p></div>{[["ИГЭ", report.ige?.map((item) => `${item.code || "Без кода"}: ${item.description || "без описания"}`).join(" · ")], ["Скважины", report.boreholes?.map((item) => `${item.name || "Без названия"}${item.depth != null ? ` — ${item.depth} м` : ""}`).join(" · ")], ["Грунтовые воды", report.groundwater?.map((item) => `${item.borehole || ""}${item.depth != null ? ` — ${item.depth} м` : ""}${item.note ? `, ${item.note}` : ""}`).join(" · ")], ["Риски", report.risks?.map((item) => `${item.severity || ""}: ${item.text || ""}`).join(" · ")]].map(([title, content]) => <div key={title as string} className="rounded-xl bg-slate-50 p-4"><h3 className="text-sm font-semibold text-slate-900">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{content || "Не извлечено"}</p></div>)}</div> : <div className="grid min-h-56 place-items-center px-6 py-12 text-center"><div className="max-w-md"><span className="mx-auto flex size-12 items-center justify-center rounded-xl bg-slate-100 text-slate-500"><FileSearch className="size-6" /></span><h3 className="mt-4 font-semibold text-slate-900">{status === "done" ? "Структурированный отчёт не получен" : "Результат появится после обработки"}</h3><p className="mt-2 text-sm leading-6 text-slate-500">Автоматические выводы не заменяют инженерную проверку.</p></div></div>}</section>
  </div>;
}
