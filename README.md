...
IFC-просмотрщик использует That Open Components и web-ifc. Пользователь выбирает локальный `.ifc`, файл проверяется по размеру и STEP-заголовку, преобразуется в Fragments и отображается в WebGL-сцене. На текущем этапе файл не отправляется в Supabase Storage или IFC worker; доступны орбитальная камера, сетка и вписывание модели. Лимит локального файла — 250 МБ.
...
Не реализованы прикладные таблицы и RLS-политики, Google OAuth, повторная отправка письма подтверждения, управление проектами, n8n, OpenAI, IFC-дерево, свойства, выбор элементов, измерения, секущие плоскости, workers, загрузка PDF/IFC в Storage, ИИ-чат, геологический анализ и калькуляторы. Supabase используется для проверки доступности и email-аутентификации; создание проекта остаётся статической заглушкой. Существующий сайт `ustabim.online` не изменяется.
