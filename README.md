...
Сейчас в репозитории реализован запускаемый frontend-прототип на Next.js, подключение Supabase SDK, регистрацию с подтверждением email, вход по email/паролю и Google OAuth, восстановление пароля через одноразовую PKCE-ссылку, read-only профиль Auth-пользователя, проекты с совместными IFC-моделями, frontend ИИ-чата и frontend анализа геологии. `/app` и форма нового пароля защищены проверенной cookie-сессией. AI backend, расчёты и серверная обработка IFC ещё не реализованы.
...
| `/app/ifc` | Защищённый локальный либо проектный просмотр IFC в браузере |
...
IFC-просмотрщик использует That Open Components и web-ifc. Локальный `.ifc` либо модель проекта проверяется по размеру и STEP-заголовку, преобразуется в Fragments и отображается в WebGL-сцене. Владелец и редактор загружают IFC до 250 МБ напрямую в приватный Supabase Storage по временной signed URL; после серверной проверки объекта участники открывают его по отдельной короткоживущей signed URL. Серверная обработка IFC и worker пока не реализованы.
...
Frontend проектов создаёт и показывает доступные проекты через `/api/projects`. Миграции создают `projects` и `project_members`, а также предоставляют роли `authenticated` ограниченный RLS-доступ: автор становится владельцем автоматически, а пользователь видит только собственные и совместные проекты. Перед использованием примените все миграции Supabase из `supabase/migrations/` в порядке имени; миграции участников поддерживают email/password и Google OAuth-аккаунты.
...
Не реализованы email-уведомления, повторная отправка письма подтверждения, хранение истории и вложений чата, скрытие/изоляция IFC-элементов, сохранённые виды, geology workflow, workers, загрузка PDF в Storage, серверное извлечение геологических данных и калькуляторы. Владелец может добавлять, менять роль и удалять участников проекта; поддерживаются зарегистрированные email/password и Google OAuth пользователи. Для чата реализован только синхронный защищённый вызов отдельного n8n webhook без памяти, RAG и project context. Supabase используется для проверки доступности, email-аутентификации, Google OAuth, проектов и приватного IFC Storage. Существующий сайт `ustabim.online` не изменяется.
