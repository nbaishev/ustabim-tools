...
| `/app/projects` | Защищённый frontend списка и создания проектов без записи в БД |
...
Frontend проектов содержит нулевую статистику без фиктивных объектов, поиск, фильтры по отношению пользователя, форму нового проекта и описание будущих разделов. Проект будет единым контуром доступа для участников, приватных файлов, IFC, геологии, чатов и расчётов. До реализации `projects`/`project_members`, API и RLS форма сохраняет введённый черновик только в состоянии страницы и явно отказывается имитировать создание.
...
Не реализованы прикладные таблицы и RLS-политики, сохранение/совместный доступ проектов, повторная отправка письма подтверждения, хранение истории и вложений чата, скрытие/изоляция IFC-элементов, сохранённые виды, geology workflow, workers, загрузка PDF/IFC в Storage, серверное извлечение геологических данных и калькуляторы. Для чата реализован только синхронный защищённый вызов отдельного n8n webhook без памяти, RAG и project context. Supabase используется для проверки доступности, email-аутентификации и Google OAuth; форма проекта остаётся frontend-заглушкой. Существующий сайт `ustabim.online` не изменяется.
