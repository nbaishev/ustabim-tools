...
Не реализованы email-уведомления и управление существующими участниками проектов, повторная отправка письма подтверждения, хранение истории и вложений чата, скрытие/изоляция IFC-элементов, сохранённые виды, geology workflow, workers, загрузка PDF в Storage, серверное извлечение геологических данных и калькуляторы. Владелец может добавить в проект любого уже зарегистрированного пользователя с email, включая пользователя Google OAuth. Для чата реализован только синхронный защищённый вызов отдельного n8n webhook без памяти, RAG и project context. Supabase используется для проверки доступности, email-аутентификации, Google OAuth, проектов и приватного IFC Storage. Существующий сайт `ustabim.online` не изменяется.
