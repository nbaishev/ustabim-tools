...
| `/app/chat` | Защищённый frontend ИИ-чата без подключения модели |
...
Frontend ИИ-чата предоставляет адаптивный интерфейс со списком диалогов, стартовыми запросами и формой сообщения. История, вложения и отправка намеренно не имитируются: до реализации защищённого API, таблиц/RLS и n8n попытка отправки показывает явное уведомление и сохраняет черновик локально в состоянии страницы.
...
Не реализованы прикладные таблицы и RLS-политики, сохранение/совместный доступ проектов, Google OAuth, повторная отправка письма подтверждения, n8n, OpenAI/AI backend, хранение истории и вложений чата, скрытие/изоляция IFC-элементов, сохранённые виды, workers, загрузка PDF/IFC в Storage, серверное извлечение геологических данных и калькуляторы. Supabase используется для проверки доступности и email-аутентификации; форма проекта остаётся frontend-заглушкой. Существующий сайт `ustabim.online` не изменяется.
